...
internal sealed class EjecutorRemoto(SshCommandRunner runner)
    : IEjecutorConPrivilegios, ISeguidorDeRegistro
{
    public Task<IAsyncDisposable> SeguirAsync(
        string command,
        Action<string> onLinea,
        Action<string?> onCerrado,
        CancellationToken ct = default) =>
        runner.SeguirAsync(command, onLinea, onCerrado, ct);
    ...
    public async Task<(bool Success, string Output, string Error)> RunWithSudoAsync(
        string command, int timeoutSeconds, CancellationToken ct = default)
    {
        var r = await runner
...
/// <summary>Lee con privilegios: el canario de <see cref="EscaladaDeLectura"/> es lo que hace que el reintento con <c>sudo</c> llegue a ejecutarse. Sólo lecturas.</summary>
[SupportedOSPlatform("windows")]
internal sealed class EjecutorConSudo(SshCommandRunner runner) : IEjecutorConPrivilegios
{
    // Las dos van por LeerAsync a proposito: este adaptador escala siempre, y por eso quien pide
    // sin privilegios recibe lo mismo que quien los pide.
...
public partial class SessionView
{
    ...
    /// <summary>Una sola muestra de procesos para los dos paneles que la miran.</summary>
    ...
    /// <summary>La contraseña de <c>sudo</c> vive en la sesión SSH cuando hay una, y sólo así se borra al cerrarla; un canal de comandos suelto lleva la suya.</summary>
    ...
    /// <summary>Un solo sondeo de <c>sudo</c> por sesión, compartido por todos los paneles.</summary>
    ...
    /// <summary>El proceso de un puerto ajeno no se ve sin privilegios: ahí va el botón.</summary>
    ...
    private async Task ReabrirLosPuertosConPrivilegiosAsync()
    {
        ...
        var lector = (IEjecutorRemoto)new EjecutorConSudo(comandos);
    ...
    /// <summary>Arma un túnel al puerto que se eligió en el panel de puertos.</summary>
    ...
    /// <summary>Abre la ficha de un proceso elegido en el panel de estado.</summary>
    ...
    /// <summary>Levanta los túneles marcados para iniciarse con la sesión y avisa cuáles no pudieron.</summary>
    private async Task LevantarTunelesAutomaticosAsync()
    {
        ...
        try
        {
            ...
            if (!_tunelesDefinidos.Any(t => t.AutoStart))
            {
                return;
            }

            var errores = await AnfitrionDeTuneles()
                .StartAutoAsync(_tunelesDefinidos).ConfigureAwait(true);

            if (errores.Count > 0)
            {
                Informar($"Túneles que no se levantaron · {string.Join(" · ", errores)}");
            }
        }
        catch (Exception ex)
        {
