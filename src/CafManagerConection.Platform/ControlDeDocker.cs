...
public sealed class ControlDeDocker
{
    ...
    private readonly ISeguidorDeRegistro? _streamer;
        ...
        Guid connectionId = default)
    {
        ...
        _streamer = runner as ISeguidorDeRegistro;
    }

    public bool PuedeSeguirRegistro => _streamer is not null;

    /// <summary>Cada línea nueva del registro del contenedor llega a <paramref name="onLinea"/>.</summary>
        ...
        AccionDeContenedor accion, string contenedor, CancellationToken ct = default)
    {
        ...
        // Se registra el verbo y no el contenedor: el nombre es contenido de sesión.
        ...
        string contenedor, CancellationToken ct = default)
    {
        ...
        var guion = string.Join('\n', [
            $"echo '{Marca.Resumen}'",
            // Las variables de entorno no se piden: ahí viven las contraseñas de base y las claves de API.
