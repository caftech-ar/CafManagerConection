...
// Interpretación y comparación de números de versión: comparar como texto ordenaría
...
/// <summary>Buscar actualizaciones a mano manda sobre el «ver después» de antes. El defecto: posponer dejaba el aviso apagado y ningún camino lo volvía a encender.</summary>
public sealed class PosposicionLevantadaAManoTests
{
    private static readonly VersionDeAplicacion Disponible = VersionDeAplicacion.Parse("0.2.0");

    [Fact]
    public void Pospuesta_hoy_no_avisa()
    {
        var ahora = DateTimeOffset.Parse("2026-09-04T10:00:00-03:00", CultureInfo.InvariantCulture);

        Assert.False(PoliticaDeActualizaciones.CorrespondeAvisar(
            Disponible, Disponible, ahora.AddHours(-2), ahora));
    }

    [Fact]
    public void Sin_posposicion_guardada_vuelve_a_avisar()
    {
        var ahora = DateTimeOffset.Parse("2026-09-04T10:00:00-03:00", CultureInfo.InvariantCulture);

        // Es el estado en el que queda tras buscar actualizaciones a mano: se borran las dos marcas.
        Assert.True(PoliticaDeActualizaciones.CorrespondeAvisar(Disponible, null, null, ahora));
    }

    [Fact]
    public void Reiniciar_la_aplicacion_el_mismo_dia_no_alcanza_para_que_vuelva()
    {
        // Construido en la zona local: CorrespondeAvisar compara LocalDateTime.Date, y las 23:00 con un desfase fijo caían al día siguiente en un runner UTC.
        var dia = new DateTime(2026, 9, 4, 23, 0, 0);
        var ahora = new DateTimeOffset(dia, TimeZoneInfo.Local.GetUtcOffset(dia));

        Assert.False(PoliticaDeActualizaciones.CorrespondeAvisar(
            Disponible, Disponible, ahora.AddHours(-13), ahora));
    }
}
