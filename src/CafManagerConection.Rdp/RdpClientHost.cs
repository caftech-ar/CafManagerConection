...
public sealed partial class RdpClientHost : AxHost
{
    ...
    /// <summary>Fija una propiedad de <see cref="IMsRdpExtendedSettings"/> (escala/DPI), que no es el dispatch por defecto. Devuelve <c>false</c> si esta versión del control no expone la interfaz o la propiedad.</summary>
    internal bool TrySetExtended(string property, object? value)
    {
        if (Ocx is not IMsRdpExtendedSettings extendida)
        {
            return false;
        }
        ...
        {
            var caja = value;
            extendida.set_Property(property, ref caja);
            return true;
        }
...
/// <summary>Escala/DPI del control RDP. No es el dispatch por defecto, por eso hay que declararla y hacer QI.</summary>
// El orden propput-antes-de-propget es el del type library de mstscax: invertirlo llama otra ranura del vtable.
[ComImport]
...
{
    void set_Property(
        [In, MarshalAs(UnmanagedType.BStr)] string bstrPropertyName,
        [In, MarshalAs(UnmanagedType.Struct)] ref object? pvarProperty);

    [return: MarshalAs(UnmanagedType.Struct)]
    object? get_Property([In, MarshalAs(UnmanagedType.BStr)] string bstrPropertyName);
}
