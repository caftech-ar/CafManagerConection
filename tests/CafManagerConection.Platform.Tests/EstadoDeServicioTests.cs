...
{
    private sealed class RunnerFalso(string salida) : IEjecutorConPrivilegios
    {
    ...
    private sealed class RunnerCaptura(Action<string> capturar) : IEjecutorConPrivilegios
    {
        public Task<(bool Success, string Output, string Error)> RunAsync(
            string command, int timeoutSeconds, CancellationToken ct = default)
        {
