...
// Sólo lectura: no hay matar, ni señalar, ni cambiar prioridad; un kill equivocado no tiene deshacer.
// Cada dato se lee por separado: sin permisos los enlaces de /proc fallan y el resto de la ficha sale igual.
public sealed record DetalleDeProceso
{
    ...
    public static DetalleDeProceso Interpretar(int pid, string nombre, string salida)
    {
        ...
        // ps es lo único que necesita existir: sin su línea, o el proceso ya no está o no hay permiso ni para verlo.
        ...
        // Un tramo vacío también es un fallo de lectura: readlink sobre el /proc de un proceso ajeno no imprime nada.
