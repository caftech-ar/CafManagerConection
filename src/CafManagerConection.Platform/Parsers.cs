...
/// <summary>Interpreta <c>docker ps</c> pedido en JSON: la salida en tabla trunca los nombres largos.</summary>
...
public static class ComposeParser
{
    ...
    /// <summary>Relaciona servicios y contenedores por el nombre <c>proyecto-servicio-N</c> de Compose v2.</summary>
...
/// <summary>Interpreta <c>nginx -T</c>: la configuración efectiva, con los <c>include</c> ya resueltos.</summary>
...
{
    private const string MarcaDeDuenos = "cmc:duenos";
    ...
    public static IReadOnlyList<ListeningPort> Parse(string salida)
    {
        ...
        var completa = salida.ReplaceLineEndings("\n");
        var corte = completa.IndexOf(MarcaDeDuenos, StringComparison.Ordinal);

        var duenos = corte < 0
            ? []
            : LeerDuenos(completa[(corte + MarcaDeDuenos.Length)..]);

        // Sin la marca, la salida es sólo la de ss: es lo que llega desde una prueba o desde un
        // servidor donde el bloque de dueños no se pudo emitir.
        foreach (var linea in (corte < 0 ? completa : completa[..corte]).Split('\n'))
        {
            var fila = linea.Trim();

            if (fila.Length == 0 ||
                fila.StartsWith("Netid", StringComparison.OrdinalIgnoreCase) ||
                fila.StartsWith("Proto", StringComparison.OrdinalIgnoreCase) ||
                fila.StartsWith("Active", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }

            var partes = fila.Split(' ', StringSplitOptions.RemoveEmptyEntries);
            ...
            var pid = LeerPid(fila);
            var dueno = pid is { } p && duenos.TryGetValue(p, out var d)
                ? d
                : (Usuario: string.Empty, Cwd: string.Empty);

            resultado.Add(new ListeningPort(
                protocolo, direccion, puerto, proceso, pid, dueno.Usuario, dueno.Cwd));
        }
    ...
    /// <summary>El bloque que el guión emite después de la marca: pid, usuario y directorio de
    /// trabajo, separados por tabulador. Los dos últimos vienen vacíos cuando el servidor no dejó
    /// leer el <c>/proc</c> de un proceso ajeno.</summary>
    private static Dictionary<int, (string Usuario, string Cwd)> LeerDuenos(string bloque)
    {
        var duenos = new Dictionary<int, (string, string)>();

        foreach (var linea in bloque.Split('\n'))
        {
            var campos = linea.Split('\t');

            if (campos.Length < 3 || !int.TryParse(campos[0].Trim(), out var pid))
            {
                continue;
            }

            duenos[pid] = (campos[1].Trim(), campos[2].Trim());
        }
    ...
    /// <summary>PID dueño del socket: <c>pid=1234</c> en <c>ss</c>, <c>1234/nginx</c> en <c>netstat</c>.</summary>
