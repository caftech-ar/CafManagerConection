...
public class FlujoCompletoTests : IDisposable
{
    ...
    private readonly List<ReferenciaDeSecreto> _credencialesCreadas = [];
    ...
    public FlujoCompletoTests()
    {
        ...
        _credentials = new VaultCredentialStore(
            new Vault(new RepositorioDelVault(_db.Factory)));
    ...
    public async Task La_busqueda_encuentra_por_host_y_por_usuario_efectivo()
    {
        ...
        // Busca por el usuario heredado, no solo por el propio.
    ...
    public async Task Un_nombre_repetido_en_la_misma_carpeta_se_detecta_pero_no_impide_guardar()
    {
        ...
        Assert.True(result.Success); // Se advierte, no se impide
    }
    ...
    public async Task Guardar_una_credencial_la_deja_en_el_almacen_y_no_en_la_base()
    {
        ...
        var referencia = ReferenciaDeSecreto.DeConexion(c.Id, Protocol.Ssh);
        _credencialesCreadas.Add(referencia);

        var guardada = await _credentials.ReadAsync(referencia);
        Assert.Equal(secreto, new string(guardada!));
    }

    [Fact]
    public async Task Borrar_una_conexion_se_lleva_su_contrasena()
    {
        var c = new Connection(Guid.NewGuid(), "Servidor", Protocol.Ssh, "192.0.2.1");
        var referencia = ReferenciaDeSecreto.DeConexion(c.Id, Protocol.Ssh);
        ...
        Assert.True(await _credentials.ExistsAsync(referencia));

        var result = await _connections.DeleteAsync(c.Id);

        Assert.True(result.Success);
        Assert.False(await _credentials.ExistsAsync(referencia));
        Assert.Null(await _connections.GetDetailAsync(c.Id));
    }

    public void Dispose()
    {
        foreach (var referencia in _credencialesCreadas)
        {
            try
            {
                _credentials.DeleteAsync(referencia).GetAwaiter().GetResult();
            }
            catch (Exception)
            {
            }
