...
public class FolderRepositoryTests
{
    ...
    [Fact]
    public async Task Persiste_los_campos_reservados_de_la_carpeta()
    {
        var (db, repo) = await CreateAsync();
        using var _ = db;
        var folder = new Folder(Guid.NewGuid(), "Producción");
        folder.Settings.CustomFields[AjustesReservados.ModoDeTamano] =
            ModoDeTamanoRdp.RenegociarResolucion.ToString();
        folder.Settings.CustomFields[AjustesReservados.EscalaDeEscritorio] = "150";
        ...
        Assert.Equal(
            ModoDeTamanoRdp.RenegociarResolucion.ToString(),
            r.Settings.CustomFields[AjustesReservados.ModoDeTamano]);
        Assert.Equal("150", r.Settings.CustomFields[AjustesReservados.EscalaDeEscritorio]);
    }

    [Fact]
    public async Task Persiste_el_usuario_y_el_puerto_por_protocolo()
    {
        var (db, repo) = await CreateAsync();
        using var _ = db;
        var folder = new Folder(Guid.NewGuid(), "Producción")
        ...
        Assert.NotNull(r);
        Assert.Equal("admin", r.Settings.RdpUserName);
        Assert.Equal("root", r.Settings.SshUserName);
        Assert.Equal(3390, r.Settings.RdpPort);
        Assert.Equal(2222, r.Settings.SshPort);
        Assert.Null(r.Settings.WebUserName);
    }
    ...
    {
        // El repositorio no borra secretos: devuelve los ids para que la capa de aplicacion
        // los saque del vault.
        var (db, repo) = await CreateAsync();
        using var _ = db;
        var carpeta = new Folder(Guid.NewGuid(), "Producción");
