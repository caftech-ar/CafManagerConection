...
public sealed class ConnectionService
{
    ...
    /// <summary>Apaga la marca que la escondía del árbol; el resto lo define el editor de siempre.</summary>
    ...
    /// <summary>Busca también en descripción, etiqueta y entorno por su nombre, no por la sigla.</summary>
        ...
        CancellationToken ct = default)
    {
        ...
        {
            record.Connection.TieneSecreto = true;
        ...
        // Despues del alta y no antes: el secreto es una columna de la conexion, asi que antes de
        // que la fila exista no hay donde escribirlo y el UPDATE no afecta a nadie.
        var donde = ReferenciaDeSecreto.DeConexion(
            record.Connection.Id, record.Connection.Protocol);

        try
        {
            await _credentials.WriteAsync(donde, credential.Secret.ToArray(), ct)
                .ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            // La conexion quedaria diciendo que tiene contraseña sin tenerla, y eso no se ve hasta
            // que falla la sesion. Se deshace el alta entera.
            await CompensarAltaAsync(record.Connection.Id, ex).ConfigureAwait(false);
            throw;
        }
        ...
        CancellationToken ct = default)
    {
        ...
        var teniaSecreto = record.Connection.TieneSecreto;
        ReferenciaDeSecreto? secretoNuevo = null;

        if (credential is not null)
        {
            var referencia = ReferenciaDeSecreto.DeConexion(
                record.Connection.Id, record.Connection.Protocol);

            var existiaAntes = await _credentials.ExistsAsync(referencia, ct).ConfigureAwait(false);

            await _credentials.WriteAsync(referencia, credential.Secret.ToArray(), ct)
                .ConfigureAwait(false);
        ...
        {
            record.Connection.TieneSecreto = teniaSecreto;

            if (secretoNuevo is { } referencia)
            {
                await CompensarCredencialAsync(referencia, ex).ConfigureAwait(false);
            }
    ...
    private async Task CompensarAltaAsync(Guid conexion, Exception causa)
    {
        try
        {
            await _connections.DeleteAsync(conexion, CancellationToken.None).ConfigureAwait(false);
        }
        catch (Exception falla)
        {
            _registro?.TechnicalError(
                $"deshacer el alta de la conexión {conexion:D} tras fallar el guardado de su "
                + "contraseña", falla);

            causa.Data[DatoDeCredencialHuerfana] = conexion.ToString("D");
        }
    }

    private async Task CompensarCredencialAsync(ReferenciaDeSecreto referencia, Exception causa)
    {
        try
        {
            await _credentials.DeleteAsync(referencia, CancellationToken.None)
                .ConfigureAwait(false);
        }
        catch (Exception falla)
        {
            // La referencia dice donde esta el secreto, no cual es: registrarla es lo que permite
            // limpiarlo.
            _registro?.TechnicalError($"limpiar la contraseña huérfana de {referencia}", falla);
            causa.Data[DatoDeCredencialHuerfana] = referencia.ToString();
        }
    }

    /// <summary>El secreto está en la fila de la conexión, así que se va con ella y no puede quedar huérfano.</summary>
    ...
    public async Task<OperationResult> ClearCredentialAsync(Guid id, CancellationToken ct = default)
    {
        ...
        // Sin clave logica el secreto es de esta fila y de ninguna otra: no hay que averiguar si
        // alguien mas lo usa.
        await _credentials.DeleteAsync(
            ReferenciaDeSecreto.DeConexion(record.Connection.Id, record.Connection.Protocol), ct)
            .ConfigureAwait(false);

        record.Connection.TieneSecreto = false;
        await _connections.UpdateAsync(record, ct).ConfigureAwait(false);
        return OperationResult.Ok();
    }
    ...
    public async Task<bool> HasStoredCredentialAsync(Guid id, CancellationToken ct = default)
    {
        ...
        return record is not null
               && record.Connection.TieneSecreto
               && await _credentials.ExistsAsync(
                   ReferenciaDeSecreto.DeConexion(
                       record.Connection.Id, record.Connection.Protocol), ct)
                   .ConfigureAwait(false);
    }

    /// <summary>Se advierte, no se impide.</summary>
    ...
    public async Task<OperationResult<Guid>> DuplicateAsync(Guid id, CancellationToken ct = default)
    {
        ...
        if (o.TieneSecreto)
        {
            var secreto = await _credentials
                .ReadAsync(ReferenciaDeSecreto.DeConexion(o.Id, o.Protocol), ct)
                .ConfigureAwait(false);
            ...
            if (secreto is not null)
            {
                try
                {
                    await _credentials.WriteAsync(
                        ReferenciaDeSecreto.DeConexion(copia.Id, copia.Protocol), secreto, ct)
                        .ConfigureAwait(false);

                    copia.TieneSecreto = true;
                }
                finally
                {
                    Array.Clear(secreto);
    ...
    /// <summary>Mueve la conexión y le deja su lugar dentro de la carpeta; sin <paramref name="posicion"/> va al alfabético.</summary>
    ...
    /// <summary>Cuelga una conexión de otra, o la suelta si <paramref name="parentId"/> es <c>null</c>.</summary>
