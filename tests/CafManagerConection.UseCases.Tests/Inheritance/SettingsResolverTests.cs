...
public class SettingsResolverTests
{
    ...
    public void Cada_protocolo_hereda_la_credencial_de_su_propio_tipo()
    {
        ...
        Assert.Equal(
            ReferenciaDeSecreto.DeCarpeta(carpeta.Id, Protocol.Rdp), rdp.Secreto.Value);
        Assert.Equal(
            ReferenciaDeSecreto.DeCarpeta(carpeta.Id, Protocol.Ssh), ssh.Secreto.Value);
    }

    [Fact]
    public void Una_conexion_web_no_hereda_la_contrasena_ssh_de_su_carpeta()
    {
        var carpeta = new Folder(Guid.NewGuid(), "Producción")
        {
            Settings = { SshTieneSecreto = true },
        };
    ...
    [Fact]
    public void Veinte_conexiones_heredan_la_contrasena_de_su_carpeta()
    {
        var carpeta = new Folder(Guid.NewGuid(), "Producción")
        {
        ...
        {
            Assert.Equal(
                ReferenciaDeSecreto.DeCarpeta(carpeta.Id, Protocol.Ssh), e.Secreto.Value);
    ...
    [Fact]
    public void Mover_a_una_carpeta_con_otra_contrasena_se_detecta_como_cambio()
    {
        ...
        Assert.Contains(cambios, c => c.Contains("contraseña", StringComparison.OrdinalIgnoreCase));
