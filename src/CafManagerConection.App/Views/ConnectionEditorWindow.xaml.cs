...
public partial class ConnectionEditorWindow : Window
{
    ...
    /// <summary>El árbol completo, para resolver de qué carpeta viene cada heredado.</summary>
    private IReadOnlyList<Folder> _todasLasCarpetas = [];

    /// <summary>Ordinal del último chequeo de nombre duplicado pedido; una respuesta con un ordinal viejo se descarta.</summary>
    ...
    private void ActualizarVisibilidad()
    {
        ...
        _bloqueRdpAvanzadoIzq.Visibility = esRdp ? Visibility.Visible : Visibility.Collapsed;
    ...
    /// <summary>Recalcula, para cada campo heredable, el valor efectivo con el que se va a conectar mientras quede en blanco (o indeterminado, para los de tres estados) y de qué carpeta sale.</summary>
    private void ActualizarHeredados()
    {
        ...
        MostrarHeredadoTexto(
            _usuarioHeredado, _usuario.Text, ancestry, f => f.Settings.UsuarioDe(Elegido));
        ...
        MostrarHeredadoValor(
            ...
            f => f.Settings.PuertoDe(Elegido), v => v.ToString());
        MostrarHeredadoTexto(
        ...
        MostrarHeredadoReservado(
            _rdpModoTamanoHeredado, _rdpModoTamano.SelectedIndex > 0, ancestry,
            AjustesReservados.ModoDeTamano, NombreDeModo);
        MostrarHeredadoReservado(
            _rdpTipoRedHeredado, _rdpTipoRed.SelectedIndex > 0, ancestry,
            AjustesReservados.TipoDeRed, NombreDeTipoDeRed);
        MostrarHeredadoReservado(
            _rdpEscalaEscritorioHeredado, _rdpEscalaEscritorio.SelectedIndex > 0, ancestry,
            AjustesReservados.EscalaDeEscritorio, v => $"{v} %");
        MostrarHeredadoReservado(
            _rdpEscalaDispositivoHeredado, _rdpEscalaDispositivo.SelectedIndex > 0, ancestry,
            AjustesReservados.EscalaDeDispositivo, v => $"{v} %");
        MostrarHeredadoReservado(
            _rdpProgramaInicialHeredado, _rdpProgramaInicial.Text.Trim().Length > 0, ancestry,
            AjustesReservados.ProgramaInicial, v => v);
    ...
    /// <summary>Campo heredable guardado como campo reservado <c>cmc:</c> en las carpetas: se muestra mientras el propio no esté definido.</summary>
    internal static void MostrarHeredadoReservado(
        TextBlock destino, bool propioDefinido, IReadOnlyList<Folder> ancestry,
        string clave, Func<string, string> formatear)
    {
        if (propioDefinido)
        {
            destino.Visibility = Visibility.Collapsed;
        ...
        foreach (var folder in ancestry)
        {
            if (folder.Settings.CustomFields.TryGetValue(clave, out var valor)
    ...
    internal static string NombreDeModo(string almacenado) =>
        AjustesReservados.ParseModoDeTamano(almacenado) switch
        {
            ModoDeTamanoRdp.Ninguno => "sin ajustar",
            ModoDeTamanoRdp.EscalarPixeles => "escalar los píxeles",
            ModoDeTamanoRdp.RenegociarResolucion => "renegociar la resolución",
            _ => almacenado,
        };

    private static string NombreDeTipoDeRed(string almacenado) =>
        AjustesReservados.ParseTipoDeRed(almacenado) switch
        {
            TipoDeRedRdp.Automatico => "automático",
            TipoDeRedRdp.Lan => "LAN",
            TipoDeRedRdp.BandaAnchaAlta => "banda ancha alta",
            TipoDeRedRdp.BandaAnchaBaja => "banda ancha baja",
            TipoDeRedRdp.Wan => "WAN",
            TipoDeRedRdp.Satelital => "satelital",
            TipoDeRedRdp.Modem => "módem",
            _ => almacenado,
        };
    ...
    /// <summary>Avisa si ya hay otra conexión con el mismo nombre en la misma carpeta. Es un aviso y no un bloqueo: nunca deshabilita Guardar, sólo informa para que quien edita decida si el nombre repetido es un error o no.</summary>
    ...
    {
        // Definir una clave privada implica querer autenticar con ella: sólo pisa «Automático» o «Contraseña», nunca una elección explícita de clave.
        if (_claveSshRuta.Text.Trim().Length > 0 && _sshMetodoAuth.SelectedIndex is 0 or 1)
        {
    ...
    private bool _verClave;

    /// <summary>Muestra u oculta la contraseña. Revelar es sólo lectura: la fuente sigue siendo el <see cref="PasswordBox"/>, así que un secreto guardado que se revela no se vuelve a guardar salvo que el usuario lo teclee.</summary>
    private async void AlAlternarVerClave(object sender, RoutedEventArgs e)
    {
        ...
        if (_clave.Password.Length > 0)
        {
            MostrarClave(_clave.Password);
            return;
        }

        if (_editando is { } id && _registro?.Connection.TieneSecreto == true)
        {
            var secreto = await LectorDeSecreto
                .LeerAsync(_root, this, ReferenciaDeSecreto.DeConexion(id, Elegido))
                .ConfigureAwait(true);

            if (secreto is not null)
            {
                MostrarClave(secreto);
            }
        }
    }

    private void MostrarClave(string texto)
    {
        _verClave = true;
        _claveVisible.Text = texto;
        _claveVisible.Visibility = Visibility.Visible;
        _clave.Visibility = Visibility.Collapsed;
    }
    ...
    private void OcultarClave()
    {
        _verClave = false;
        _claveVisible.Clear();
        _claveVisible.Visibility = Visibility.Collapsed;
        _clave.Visibility = Visibility.Visible;
    }
    ...
    private void AplicarCatalogo(Connection c)
    {
        ...
        {
            AjustesReservados.EscribirOpcionesDePantalla(
                LeerOpcionesDePantalla(), c.SetCustomField);
        }
    }

    internal static readonly TipoDeRedRdp?[] TiposDeRed =
    [
        null, TipoDeRedRdp.Automatico, TipoDeRedRdp.Lan, TipoDeRedRdp.BandaAnchaAlta,
        TipoDeRedRdp.BandaAnchaBaja, TipoDeRedRdp.Wan, TipoDeRedRdp.Satelital, TipoDeRedRdp.Modem,
    ];

    internal static readonly ModoDeTamanoRdp?[] ModosDeTamano =
    [
        null, ModoDeTamanoRdp.Ninguno, ModoDeTamanoRdp.EscalarPixeles,
        ModoDeTamanoRdp.RenegociarResolucion,
    ];

    internal static readonly int?[] EscalasEscritorio = [null, 100, 125, 150, 175, 200];

    internal static readonly int?[] EscalasDispositivo = [null, 100, 140, 180];

    /// <summary>Reúne las opciones de pantalla que el formulario tiene cargadas.</summary>
    private OpcionesDePantallaRdp LeerOpcionesDePantalla()
    {
        ...
        return new OpcionesDePantallaRdp(
            rendimiento,
            TiposDeRed[Math.Max(_rdpTipoRed.SelectedIndex, 0)],
            ModosDeTamano[Math.Max(_rdpModoTamano.SelectedIndex, 0)],
            EscalasEscritorio[Math.Max(_rdpEscalaEscritorio.SelectedIndex, 0)],
            EscalasDispositivo[Math.Max(_rdpEscalaDispositivo.SelectedIndex, 0)],
            _rdpProgramaInicial.Text,
            _rdpDirectorioTrabajo.Text,
            _rdpRemoteApp.IsChecked == true);
    }

    /// <summary>Vuelca al formulario las opciones de pantalla propias de la conexión; los combos en «Heredar» cuando no hay valor propio.</summary>
    private void VolcarOpcionesDePantalla(Connection c)
    {
        var o = AjustesReservados.LeerOpcionesDePantalla(c.CustomFields);
        ...
        _rdpModoTamano.SelectedIndex = Math.Max(Array.IndexOf(ModosDeTamano, o.ModoDeTamano), 0);
        _rdpTipoRed.SelectedIndex = Math.Max(Array.IndexOf(TiposDeRed, o.TipoDeRed), 0);
        _rdpEscalaEscritorio.SelectedIndex = Math.Max(Array.IndexOf(EscalasEscritorio, o.EscalaDeEscritorio), 0);
        _rdpEscalaDispositivo.SelectedIndex = Math.Max(Array.IndexOf(EscalasDispositivo, o.EscalaDeDispositivo), 0);
    ...
    /// <summary>Con la identidad de Windows no se pide ni se guarda ninguna contraseña: la que no se guarda no se puede filtrar.</summary>
