...
/// <summary>Trae conexiones guardadas de PuTTY, WinSCP y FileZilla.</summary>
...
public partial class PanelImportacion : UserControl
{
    ...
    private void RevisarOrigenes()
    {
        ...
        Aplicar(
            _origenRdm,
            _detalleRdm,
            _rutaDeRdm is not null,
            _rutaDeRdm ?? "elegí el archivo exportado desde Remote Desktop Manager");
    ...
    private RadioButton[] Radios() =>
        [_origenPutty, _origenWinScpRegistro, _origenWinScpIni, _origenFileZilla, _origenRdm];
    ...
    /// <summary>Si esta máquina tiene algo para importar. Lo pregunta la ventana principal en el primer arranque: sin orígenes, ofrecer la importación es mandar a una pantalla vacía.</summary>
    public static bool HayAlgoQueImportar()
    {
        if (RutaQueExiste(LectorDeWinScp.RutasHabitualesDelIni()) is not null
            || RutaQueExiste([LectorDeFileZilla.RutaHabitual()]) is not null)
        {
            return true;
        }

        return TieneSesiones(LectorDePutty.LeerRegistro)
               || TieneSesiones(LectorDeWinScp.LeerRegistro);
    }

    // Sin registrar el fallo y sin el detalle: acá sólo se decide si vale la pena ofrecer la
    // pantalla, y el panel vuelve a probar cada origen con su mensaje cuando se abre.
    private static bool TieneSesiones(Func<LecturaDeImportacion> leer)
    {
        try
        {
            using var lectura = leer();
            return lectura.Compatibles.Count + lectura.Omitidas.Count > 0;
        }
        catch (Exception)
        {
            return false;
    ...
    private OrigenDeImportacion? OrigenElegido() =>
        ...
        : _origenRdm.IsChecked == true ? OrigenDeImportacion.Rdm
        : null;

    /// <summary>Remote Desktop Manager no guarda sus sesiones en una ruta fija: son un archivo que el usuario exporta y elige.</summary>
    private void AlElegirArchivoDeRdm(object sender, RoutedEventArgs e)
    {
        var dialogo = new Microsoft.Win32.OpenFileDialog
        {
            Title = "Archivo exportado desde Remote Desktop Manager",
            Filter = "Exportación de RDM (*.xml;*.rdm)|*.xml;*.rdm|Todos los archivos|*.*",
        };
        ...
        _rutaDeRdm = dialogo.FileName;
        _origenRdm.IsChecked = true;

        Aplicar(_origenRdm, _detalleRdm, true, _rutaDeRdm);

        _estado.Text = "Tocá «Buscar» para leer este archivo.";
    }
    ...
    private LecturaDeImportacion Leer(OrigenDeImportacion origen) => origen switch
    {
        ...
        OrigenDeImportacion.Rdm when _rutaDeRdm is { } rdm =>
            LectorDeRdm.Leer(File.ReadAllText(rdm)),
    ...
    private static string Nombre(OrigenDeImportacion origen) => origen switch
    {
        ...
        OrigenDeImportacion.Rdm => "Remote Desktop Manager",
        _ => "FileZilla",
    };
    ...
    private void Mostrar()
    {
        ...
        _cuantasContrasenas.Text = (conContrasena, OrigenElegido()) switch
        {
            ...
            (0, OrigenDeImportacion.Rdm) =>
                "Remote Desktop Manager las guarda cifradas con su propia clave: no se pueden traer.",
    ...
    private static string Resumen(ResultadoDeImportacion resultado)
    {
        ...
        if (resultado.ContrasenasPerdidas > 0)
        {
            partes.Add(
                $"Entraron sin su contraseña ({resultado.ContrasenasPerdidas}). La conexión está y "
                + "hay que volver a cargarle la contraseña:\n"
                + Recortar(resultado.ContrasenasSinTraer, 8));
        }
