...
public sealed record RdpSessionRequest(
    ...
    int? EscalaDeEscritorio = null,
    int? EscalaDeDispositivo = null,
    string? ProgramaInicial = null,
    string? DirectorioDeTrabajo = null,
    bool ComoRemoteApp = false);
...
public enum AmbitoDeRdp
{
    ...
    /// <summary><see cref="IMsRdpExtendedSettings"/>, que no es el dispatch por defecto del control: la escala vive acá.</summary>
    Extendida,
}

public readonly record struct AjusteDeRdp(AmbitoDeRdp Ambito, string Propiedad, object? Valor);

/// <summary>Qué se le asigna al ActiveX para una petición. La contraseña no viaja acá: sólo la bandera que dice si se asigna.</summary>
...
{
    /// <summary>Con la identidad de Windows no se asigna usuario, dominio ni contraseña: CredSSP delega el token de la sesión actual.</summary>
        ...
        RdpSessionRequest peticion, string? usuarioDeLaCredencial, bool hayContrasena)
    {
        ...
        // SmartSizing escala los píxeles; la resolución dinámica es lo opuesto y va por Resize.
        ajustes.Add(new(
            AmbitoDeRdp.Avanzados,
            "SmartSizing",
            peticion.ModoDeTamano == ModoDeTamanoRdp.EscalarPixeles));

        ajustes.Add(new(AmbitoDeRdp.Avanzados, "RedirectClipboard", peticion.ClipboardEnabled));

        // DisableRdpdr=1 apaga el canal de dispositivos y con él el portapapeles en varias versiones del control; sólo se apaga cuando el portapapeles no se quiere.
        ajustes.Add(new(AmbitoDeRdp.Avanzados, "DisableRdpdr", peticion.ClipboardEnabled ? 0 : 1));

        if (peticion.Rendimiento != BanderasDeRendimientoRdp.Ninguna)
        {
            ajustes.Add(new(
                AmbitoDeRdp.Avanzados, "PerformanceFlags", (int)peticion.Rendimiento));
        }

        if (peticion.TipoDeRed is { } red)
        {
            ajustes.Add(new(AmbitoDeRdp.Avanzados, "NetworkConnectionType", (int)red));
        }

        if (peticion.EscalaDeEscritorio is { } escritorio)
        {
            ajustes.Add(new(AmbitoDeRdp.Extendida, "DesktopScaleFactor", (uint)escritorio));
        }

        if (peticion.EscalaDeDispositivo is { } dispositivo)
        {
            ajustes.Add(new(AmbitoDeRdp.Extendida, "DeviceScaleFactor", (uint)dispositivo));
        }

        ajustes.AddRange(ProgramaOApp(peticion));
    ...
    /// <summary>Un programa inicial en el escritorio (StartProgram) o, si se pidió RemoteApp, el modo de programa remoto; el nombre del RemoteApp lo fija <see cref="RdpSession"/> sobre el objeto RemoteProgram2.</summary>
    private static IEnumerable<AjusteDeRdp> ProgramaOApp(RdpSessionRequest peticion)
    {
        var programa = peticion.ProgramaInicial?.Trim();
        ...
        if (peticion.ComoRemoteApp)
        {
            yield return new(AmbitoDeRdp.Avanzados, "RemoteProgramMode", true);
            yield break;
        }

        yield return new(AmbitoDeRdp.Asegurados, "StartProgram", programa);

        var directorio = peticion.DirectorioDeTrabajo?.Trim();

        if (!string.IsNullOrEmpty(directorio))
        {
            yield return new(AmbitoDeRdp.Asegurados, "WorkDir", directorio);
        }
    }

    /// <summary>Discos, audio, micrófono, impresoras, puertos y tarjetas inteligentes, apagados.</summary>
...
public sealed class RdpSession : IDisposable
{
    ...
    /// <summary>Propiedades que esta versión del ActiveX no aceptó. Nombres, nunca valores.</summary>
    ...
    /// <summary>Tras fallar la identidad de Windows, si hay que reintentar pidiendo credenciales en lugar de dar la conexión por perdida.</summary>
    ...
    /// <summary>Crea el control sin configurarlo ni conectar: hay que montarlo y mostrarlo antes de llamar a <see cref="PrepararYConectar"/>, que es quien configura y conecta.</summary>
    ...
    private void Configure(StoredCredential? credential)
    {
        ...
        ConfigurarRemoteApp(host);
        ConfigureDisplay(host);
    }

    /// <summary>El RemoteApp se nombra sobre el objeto RemoteProgram2 del control, no con una propiedad simple; sin programa o sin la casilla, no se toca.</summary>
    private void ConfigurarRemoteApp(RdpClientHost host)
    {
        var programa = _request.ProgramaInicial?.Trim();

        if (!_request.ComoRemoteApp || string.IsNullOrEmpty(programa))
        {
            return;
        }

        if (host.GetObject("RemoteProgram2") is not { } remoto)
        {
            return;
        }

        RdpClientHost.TrySetOn(remoto, "RemoteProgramName", programa);

        var directorio = _request.DirectorioDeTrabajo?.Trim();

        if (!string.IsNullOrEmpty(directorio))
        {
            RdpClientHost.TrySetOn(remoto, "RemoteProgramWorkDir", directorio);
        }
    }
    ...
    private static bool Aplicar(
        ...
        ajuste.Ambito switch
        {
            ...
            AmbitoDeRdp.Extendida => host.TrySetExtended(ajuste.Propiedad, ajuste.Valor),
    ...
    private SessionFailure MotivoDeDesconexion(RdpClientHost host)
    {
        ...
        return ConDiagnostico(ReinterpretarRemoteApp(fallo, codigo));
    }

    /// <summary>Un corte antes de conectar con la casilla de RemoteApp casi siempre es que el servidor no lo publicó; se distingue de red y credenciales, que ya tienen su causa.</summary>
    private SessionFailure ReinterpretarRemoteApp(SessionFailure fallo, int codigo)
    {
        if (!_request.ComoRemoteApp
            || LlegoAConectar
            || fallo.Reason is not (SessionFailureReason.UnexpectedDisconnect
                                    or SessionFailureReason.Other))
        {
            return fallo;
        }
        ...
        return new SessionFailure(
            SessionFailureReason.Other,
            "El programa RemoteApp no está publicado en el servidor.",
            "Pedile al administrador que lo publique, o desmarcá «abrir como RemoteApp».",
            $"ExtendedDisconnectReason={codigo}; remoteApp=1");
    }
    ...
    {
        if (_request.ModoDeTamano != ModoDeTamanoRdp.RenegociarResolucion
            || _host is not { } host || State != SessionState.Connected)
        {
            return;
        }
