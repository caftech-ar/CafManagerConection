...
public sealed class SshCommandRunner : IAsyncDisposable, ISeguidorDeRegistro
{
    ...
    /// <summary>Lo que se anota de una escalada: el verbo y el resultado, nunca el secreto que fue por la entrada estándar.</summary>
    ...
    /// <summary>Abre <paramref name="command"/> en una conexión SSH propia y va entregando líneas mientras corre: sirve igual a <c>docker logs -f</c> y a <c>tail -F</c>.</summary>
...
/// <summary>Quita de un texto la contraseña que se le pasó a <c>sudo</c>, antes de que llegue a la traza, al registro o a un mensaje de error.</summary>
...
/// <summary>El orden de la escalada: primero la contraseña de la conexión, después —una sola vez por sesión— la que escriba el usuario, y si ninguna sirve la escalada queda imposible.</summary>
