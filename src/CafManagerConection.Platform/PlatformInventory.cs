...
// Sólo lectura: no hay ningún método que escriba. Se consulta al abrir el panel y con el botón, nunca en bucle.
public sealed class PlatformInventory
{
    private const string Marca = "###CMC###";

    private readonly IEjecutorConPrivilegios _runner;
    private readonly int _timeout;
    private readonly IAppLogger? _logger;
    private readonly Guid _conexion;

    public PlatformInventory(
        IEjecutorConPrivilegios runner,
    ...
    private static readonly string GuionDeteccion = """
        ...
        # Por PID y leyendo /proc, no parseando la salida de pgrep -af. Dos motivos, y el primero
        # ya rompio una vez: pgrep -af busca en la linea de comando completa de todo el sistema,
        # y este guion se ejecuta como argumento de bash, asi que se encontraba a si mismo y
        # terminaba leyendo su propio texto como si fuera la configuracion de supervisord. Y en
        # /proc los argumentos vienen separados por NUL, de modo que el -c y su ruta son dos
        # argumentos distintos: no hay comillas de shell que interpretar y una ruta con espacios
        # llega entera.
        pid=$(pgrep -x supervisord 2>/dev/null | head -1)

        if [ -z "$pid" ]; then
            # supervisord lanzado como `python /usr/bin/supervisord`: ahi el nombre del proceso es
            # python. Se excluye el propio arbol para no volver a encontrarse a si mismo.
            pid=$(pgrep -f 'bin/supervisord' 2>/dev/null \
                  | grep -v -e "^$$\$" -e "^$PPID\$" | head -1)
        fi

        if [ -n "$pid" ] && [ -r "/proc/$pid/cmdline" ]; then
            argumentos=$(tr '\0' '\n' < "/proc/$pid/cmdline")

            conf=$(printf '%s\n' "$argumentos" | awk 'anterior=="-c"{print; exit} {anterior=$0}')

            # Ruta absoluta o nada: una relativa no sirve, porque supervisorctl corre desde otro
            # directorio de trabajo que el supervisord que la recibio.
            case "$conf" in
                /*) ;;
                *) conf="" ;;
            esac

            binario=$(printf '%s\n' "$argumentos" | grep -m1 '/supervisord$')

            if [ -n "$binario" ] && [ -x "${binario%/supervisord}/supervisorctl" ]; then
                sup="${binario%/supervisord}/supervisorctl"
            fi
        fi
        ...
        if [ -n "$sup" ] && [ -n "$conf" ]; then
            # Entre comillas: la ruta puede tener espacios, y este string se concatena mas
            # adelante con ` status` y ` tail -N proceso`.
            comando="$sup -c \"$conf\""
        else
            comando="$sup"
        fi
    ...
    /// <summary>Proyectos compose con sus servicios ya relacionados con los contenedores.</summary>
    ...
    // El dueño y el directorio de trabajo salen de /proc y no de `ss`, que no los trae. Van en
    // el mismo viaje porque sobre un enlace lento cada ida y vuelta se nota, y sólo para los PID
    // que `ss` reportó: recorrer /proc entero costaba una lectura por proceso del servidor.
    private static readonly string GuionPuertos = """
        escucha=$(ss -tulnpH 2>/dev/null || netstat -tulnp 2>/dev/null)

        printf '%s\n' "$escucha"
        echo "cmc:duenos"

        for pid in $(printf '%s\n' "$escucha" |
                     grep -oE '(pid=[0-9]+|[[:space:]][0-9]+/)' |
                     grep -oE '[0-9]+' | sort -un); do
            [ -d "/proc/$pid" ] || continue

            usuario=$(stat -c %U "/proc/$pid" 2>/dev/null)
