...
/// <summary>Preferencias, en pestañas: dónde vive todo, apariencia, etiquetas, importación, copias y ajustes avanzados.</summary>
...
public partial class PreferenciasWindow : Window
{
    ...
    public sealed record FilaCredencial(string Tipo, string Dueno, string Donde);
    ...
    public bool VolverAAvisar { get; private set; }
    ...
    /// <summary>Lista dónde hay contraseña guardada, resuelto contra el árbol. Nunca trae ningún secreto.</summary>
    private async Task RefrescarCredencialesAsync()
    {
        try
        {
            var referencias = await _root.Credentials.EnumerateAsync().ConfigureAwait(true);
            ...
            var filas = referencias
                .Select(r => Describir(r, conexiones, carpetas))
                .ToList();

            _credenciales.ItemsSource = filas;

            _resumenCreds.Text = filas.Count == 0
                ? "Todavía no guardaste ninguna."
                : $"{filas.Count} guardada(s).";
        }
        catch (Exception ex)
        {
    ...
    /// <summary>La advertencia que va antes de quitar la clave maestra.</summary>
    private bool ConfirmaQuitarLaClaveMaestra() =>
        MessageBox.Show(
            "Si quitás la clave maestra, tus contraseñas quedan guardadas en claro: cualquiera "
            + "que tenga el archivo de la base puede leerlas.\n\nSe descifran todas ahora mismo, "
            + "en una sola operación.",
            "Quitar la clave maestra",
            MessageBoxButton.OKCancel,
            MessageBoxImage.Warning) == MessageBoxResult.OK;

    // Una referencia siempre resuelve: sale de una fila que existe. La credencial huerfana dejo
    // de poder existir al mudarse el secreto a la fila que lo usa.
    private static FilaCredencial Describir(
        ReferenciaDeSecreto referencia,
        IReadOnlyDictionary<Guid, string> conexiones,
        IReadOnlyDictionary<Guid, string> carpetas)
    {
        var (tipo, nombre) = referencia.EsDeCarpeta
            ? ($"carpeta {referencia.Protocolo.ToString().ToLowerInvariant()}",
               carpetas.GetValueOrDefault(referencia.Id))
            : (referencia.Protocolo.ToString().ToUpperInvariant(),
               conexiones.GetValueOrDefault(referencia.Id));

        return new FilaCredencial(tipo, nombre ?? "—", referencia.ToString());
    }
    ...
    /// <summary>Abre la ventana ya parada en Importar. La usa el primer arranque, cuando todavía no hay ninguna conexión.</summary>
    public void MostrarImportacion() => _pestanaImportar.IsSelected = true;

    private void AlCerrar(object sender, RoutedEventArgs e) => Close();

    /// <summary>Muestra en qué estado de protección está este vault, y qué botones tienen sentido.</summary>
    private void RefrescarEstadoDelVault()
    {
        var conClave = _root.Vault.PideClaveMaestra;

        _estadoDelVault.Text = conClave
            ? "Tenés clave maestra: las contraseñas están cifradas y el archivo no se abre sin "
              + "ella en ningún equipo."
            : "No tenés clave maestra: las contraseñas se guardan en claro, así que cualquiera "
              + "que tenga el archivo puede leerlas. Poné una si el archivo va a salir de acá.";

        _botonClaveMaestra.Content = conClave ? "Cambiar la clave maestra" : "Poner una clave maestra";
        _botonBloquear.IsEnabled = conClave && _root.Vault.EstaAbierto;
    }

    private async void AlCambiarClaveMaestra(object sender, RoutedEventArgs e)
    {
        try
        {
            var conClave = _root.Vault.PideClaveMaestra;

            var ventana = new ClaveMaestraWindow(
                conClave ? ModoDeClaveMaestra.Cambiar : ModoDeClaveMaestra.Crear) { Owner = this };

            var acepto = ventana.ShowDialog() == true;

            // Con clave puesta, cerrar con «Quitar la clave maestra» es una decisión y no un
            // cancelar: sin esta rama, la advertencia no tiene ningún camino en la interfaz.
            if (!acepto && !(conClave && ventana.SinClaveMaestra))
            {
                return;
            }

            if (!acepto && !ConfirmaQuitarLaClaveMaestra())
            {
                return;
            }

            var actual = ventana.ClaveActual ?? [];
            var nueva = acepto ? ventana.Clave ?? [] : [];

            try
            {
                var hecho = await _root.Vault.DefinirClaveMaestraAsync(actual, nueva)
                    .ConfigureAwait(true);

                _estado.Text = hecho
                    ? nueva.Length == 0
                        ? "Se quitó la clave maestra. Las contraseñas quedaron en claro."
                        : "Clave maestra actualizada. Se recifraron todas las contraseñas."
                    : "La clave maestra actual no es correcta.";
            }
            finally
            {
                Array.Clear(actual);
        ...
        {
            _root.Logger.TechnicalError("cambiar la clave maestra", ex);
            _estado.Text = ex.Message;
        }
    }

    private void AlBloquearVault(object sender, RoutedEventArgs e)
    {
        _root.Vault.Bloquear();

        _estado.Text = "Vault bloqueado. La próxima contraseña que haga falta va a pedir la clave maestra.";
        RefrescarEstadoDelVault();
    }
    ...
    private async void AlBuscarActualizaciones(object sender, RoutedEventArgs e)
    {
        ...
        if (resultado is { HayVersionNueva: true, VersionDisponible: not null })
        {
            await _actualizaciones.OlvidarPosposicionAsync(_ajustesDeActualizacion)
                .ConfigureAwait(true);

            _ajustesDeActualizacion = await _actualizaciones.ObtenerAjustesAsync()
                .ConfigureAwait(true);
        ...
        _estadoActualizaciones.Text = resultado switch
        {
            ...
            { HayVersionNueva: true, VersionDisponible: { } version } =>
                $"Hay una versión nueva: {version}. El aviso vuelve a aparecer al cerrar esta ventana.",
    ...
    private async void AlElegirModoDePestana(object sender, SelectionChangedEventArgs e)
    {
        if (_cargando)
        {
            return;
