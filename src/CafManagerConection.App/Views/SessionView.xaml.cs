...
public partial class SessionView : UserControl, IHostKeyVerifier, IDisposable
{
    ...
    /// <summary>La identidad de Windows ya falló en esta pestaña: el próximo intento pide credenciales.</summary>
    ...
    /// <summary>Demora antes de aplicar lo que se escribió en la barra de búsqueda.</summary>
    ...
    /// <summary>Cuántos túneles de esta sesión están activos ahora mismo.</summary>
        ...
        EffectiveSettings efectivo)
    {
        ...
        try
        {
            var secreto = await _root.Credentials.ReadAsync(efectivo.Secreto.Value)
                .ConfigureAwait(true);
        ...
        catch (CredencialIlegibleException ex)
        {
            _root.Logger.TechnicalError("leer la contraseña guardada de la conexión", ex);
        ...
        {
            _mensaje.Text = "El vault está cerrado: la contraseña se pide para esta sesión y no "
                            + "se guarda.";
    ...
    /// <summary>Pisa con ceros la credencial descifrada que esta pestaña tenía viva. La sesión
    /// abierta sigue conectada: lo que ya se le entregó a SSH.NET o al control de RDP vive dentro
    /// de ellos y no se puede pisar desde acá.</summary>
    public void PisarCredencialViva()
    {
        _credencial?.Dispose();
        _credencial = null;
    }
    ...
    /// <summary>Se va a pegar más de una línea sin que el servidor haya pedido el modo 2004.</summary>
    ...
    private async Task OfrecerGuardarCredencialAsync()
    {
        ...
        var guardar = Services.Dialogos.Confirmar(
            ventana,
            "Guardar contraseña",
            $"¿Guardar la contraseña de «{usuario}» cifrada en el vault de CMC para no "
            + "volver a pedirla la próxima vez que se conecte?",
            "Guardar");
        ...
        try
        {
            await _root.Credentials.WriteAsync(
                ReferenciaDeSecreto.DeConexion(ConnectionId, Protocol.Ssh),
                credencial.Secret.ToArray()).ConfigureAwait(true);

            _registro.Connection.TieneSecreto = true;
        ...
        EffectiveSettings efectivo, DateTimeOffset inicio, bool conIdentidadDeWindows)
    {
        var peticion = new RdpSessionRequest(
            ...
            efectivo.ResolvedModoDeTamano,
            efectivo.ResolvedIgnoreCertificateWarnings,
            Domain.Settings.Defaults.ConnectionTimeoutSeconds,
            conIdentidadDeWindows,
            efectivo.ResolvedRendimiento,
            efectivo.ResolvedTipoDeRed,
            efectivo.ResolvedEscalaDeEscritorio,
            efectivo.ResolvedEscalaDeDispositivo,
            efectivo.ResolvedProgramaInicial,
    ...
    /// <summary>Fuera del dominio o contra un servidor que no confía, la identidad de Windows se descarta y la conexión se reintenta pidiendo credenciales en lugar de fallar.</summary>
    private bool CaerAlPedidoDeCredenciales(SessionFailure? fallo)
    {
        ...
        // La reconexión espera un turno del despachador: el fallo llega del sondeo del propio ActiveX y desarmarlo dentro de ese callback lanza InvalidComObjectException desde RdpClientHost.
    ...
    /// <summary>La sesión ocupa toda la ventana, sin el árbol al costado, y volver la deja como estaba sin reconectar: no mueve el control de lugar.</summary>
    ...
    /// <summary>Al segundo y medio se le pregunta al control si sigue conectado: si el traslado la cortó, se dice; no se reconecta por atrás.</summary>
