...
public sealed class DockerPanel : PanelInventario
{
    ...
    // docker ps contesta en unos 100 ms, el paso de compose suma otros 870 ms y stats dos segundos y medio más.
    ...
    /// <summary>Fila de un servicio declarado en el compose que nunca llegó a tener contenedor.</summary>
...
/// <summary>Sitios publicados por nginx.</summary>
[SupportedOSPlatform("windows")]
...
/// <summary>Procesos que gestiona supervisord.</summary>
[SupportedOSPlatform("windows")]
public sealed class SupervisorPanel : PanelInventario
{
    ...
    private readonly IEjecutorConPrivilegios? _canal;

    /// <param name="canal">El mismo ejecutor de la sesión; sin él el visor no puede seguir el registro en vivo.</param>
    public SupervisorPanel(
        PlatformInventory inventario,
        ControlDeSupervisor control,
        IEjecutorConPrivilegios? canal = null)
        : base("supervisord")
    {
        _inventario = inventario;
        _control = control;
    ...
    /// <summary>Abre el registro del proceso en un visor que lo sigue en vivo.</summary>
...
public sealed class PuertosPanel : PanelInventario
{
        ...
        : base("Puertos a la escucha")
    {
        ...
        ColumnaDeProceso();
        Columna("Usuario", nameof(Fila.Usuario), 0.9);
        Columna("Directorio", nameof(Fila.Directorio), 2.0);
    ...
    /// <summary>Qué contenedor publica cada puerto, para los que abrió el reenviador de Docker.</summary>
    ...
    /// <summary>Abre la ficha del proceso que tiene el puerto elegido.</summary>
        ...
        Tabla.ItemsSource = lista
            .Select(p =>
            {
                ...
                return new Fila(
                    ...
                    p.Pid,
                    EstadoDeTunel(p),
                    p.Usuario.Length > 0 ? p.Usuario : "—",
                    p.DirectorioDeTrabajo.Length > 0 ? p.DirectorioDeTrabajo : "—");
            })
            .ToList();
...
/// <summary>El registro de un proceso de supervisord: se lee con <c>supervisorctl tail</c> y se sigue con <c>tail -F</c> sobre los archivos que el proceso tiene abiertos.</summary>
[SupportedOSPlatform("windows")]
public sealed class RegistroDeProcesoSupervisado(
    PlatformInventory inventario, IEjecutorConPrivilegios? canal, string proceso)
    : IFuenteDeRegistro
{
    ...
    {
        if (canal is not ISeguidorDeRegistro seguidor)
        {
            PorQueNoSigue =
