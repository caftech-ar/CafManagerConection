...
/// <summary>Ventana principal: árbol de servidores a la izquierda, sesiones en pestañas a la derecha.</summary>
...
public partial class MainWindow : Window, ISessionHost
{
    ...
    /// <summary>Catálogo de etiquetas en orden, para el submenú del árbol.</summary>
    ...
    /// <summary>Identificadores de conexión de las conexiones rápidas abiertas en esta ventana.</summary>
    private readonly HashSet<Guid> _conexionesRapidas = [];

    /// <summary>Aviso de versión nueva: qué se está ofreciendo y con qué ajustes.</summary>
    ...
    public MainWindow(CompositionRoot root)
    {
        ...
        _root.Vault.Bloqueado += AlBloquearElVault;
        ...
        // Sin Mica: AplicarFondoMica extiende el marco de DWM a toda el area cliente y deja la
        // superficie transparente, y eso saca ClearType de todo el texto WPF y engrosa el del
        // terminal, que es una ventana GDI hija. Volver a pedirlo exige no extender el marco mas
        // alla de la barra de titulo.
        Loaded += (_, _) => Services.Temas.AplicarBarraDeTitulo(this);
    ...
    // FromVersion en 0 es la base recien creada: no hay ninguna conexion todavia. Solo se ofrece
    // si esta maquina tiene algo que leer, porque llevar a la pantalla de importacion sin ningun
    // origen es mandar a una pantalla vacia.
    private async Task OfrecerImportarEnElPrimerArranqueAsync()
    {
        if (_root.Startup.FromVersion != 0 || !PanelImportacion.HayAlgoQueImportar())
        {
            return;
        }

        var acepta = Dialogos.Confirmar(
            this,
            "Traer tus conexiones",
            "Esta máquina tiene conexiones guardadas en otro programa —PuTTY, WinSCP o "
            + "FileZilla—. ¿Querés traerlas ahora?"
            + Environment.NewLine + Environment.NewLine
            + "También podés hacerlo después, desde Preferencias, en la pestaña Importar.",
            "Importar ahora");
        ...
        ventana.MostrarImportacion();
        ventana.ShowDialog();

        await RefrescarArbolAsync().ConfigureAwait(true);
    }

    /// <summary>Bloquear el vault tiene que sacar de memoria también las credenciales que las
    /// pestañas abiertas tenían descifradas: sin esto el aviso no tiene quien lo escuche y
    /// bloquear no bloquea nada.</summary>
    private void AlBloquearElVault()
    {
        foreach (var vista in _sesiones.Items.OfType<TabItem>()
                     .Select(t => t.Content).OfType<SessionView>())
        {
            vista.PisarCredencialViva();
        }
    }

    private async Task AbrirElVaultAsync()
    {
        var aviso = await new AperturaDelVault(_root.Vault, _root.Logger, () => this)
    ...
    /// <summary>Ofrece desbloquear el vault cuando una acción lo necesita y está cerrado.</summary>
    private async Task<bool> OfrecerDesbloquearAsync()
    {
        await AbrirElVaultAsync().ConfigureAwait(true);

        return _root.Vault.EstaAbierto;
    }
    ...
    private async Task AplicarModoDePestanaAsync()
    {
        ...
        {
            ModoDePestana.EnvueltoEnFilas => "PlantillaPestanasEnvuelto",
            ModoDePestana.LinealConDesplegable => "PlantillaPestanasDesplegable",
            _ => "PlantillaPestanasLineal",
        };

        _sesiones.Template = (ControlTemplate)FindResource(clave);
    }

    /// <summary>Llena el menú de sobra con las sesiones abiertas al desplegarlo; elegir una la activa.</summary>
    private void AlAbrirSobra(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var pestana in _sesiones.Items.OfType<TabItem>())
        {
            ...
            {
                Header = _titulosDePestana.GetValueOrDefault(pestana, "Sesión"),
                IsChecked = ReferenceEquals(_sesiones.SelectedItem, pestana),
            };

            var cual = pestana;
            item.Click += (_, _) => _sesiones.SelectedItem = cual;
            menu.Items.Add(item);
        }

        if (menu.Items.Count == 0)
        {
            menu.Items.Add(new MenuItem { Header = "(sin sesiones)", IsEnabled = false });
        }
    }
    ...
    /// <summary>La etiqueta propia de cada carpeta que tenga una: la carpeta guarda el identificador y el árbol muestra el código.</summary>
    ...
    /// <summary>Título de cada pestaña, para el menú de sobra del modo con desplegable.</summary>
    private readonly Dictionary<TabItem, string> _titulosDePestana = [];
    ...
    ISessionSurface ISessionHost.Create(Guid sessionId, ConnectionRecord connection)
    {
        ...
        _titulosDePestana[pestana] = nombre;
        ...
        : ISessionSurface
    {
        ...
        public void Dispose()
        {
            ...
            finally
            {
                ...
                ventana._titulosDePestana.Remove(pestana);
    ...
    /// <summary>El título dice qué sesión está activa y cuántas hay abiertas.</summary>
    ...
    /// <summary>Abre una segunda sesión sobre la misma conexión.</summary>
    ...
    /// <summary>Confirma antes de cerrar varias sesiones de una vez.</summary>
    ...
    /// <summary>Borra del repositorio la conexión que sostenía una sesión rápida ya cerrada.</summary>
    ...
    /// <summary>Arma la frase de la advertencia previa al cierre a partir de cuántas sesiones y túneles van a cerrarse.</summary>
    ...
    /// <summary>Comprueba si hay una versión nueva, al arrancar.</summary>
    ...
    /// <summary>«Después»: pospone esta versión hasta mañana.</summary>
