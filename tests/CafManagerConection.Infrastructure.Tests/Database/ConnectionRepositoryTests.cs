...
public class ConnectionRepositoryTests
{
    ...
    public async Task Los_campos_heredables_se_guardan_como_nulos()
    {
        ...
        Assert.False(r.Connection.TieneSecreto);
        Assert.Null(r.Ssh!.AuthMethod);
        Assert.Null(r.Ssh.KeepAliveSeconds);
    }

    [Fact]
    public async Task El_repositorio_de_conexiones_no_escribe_ningun_secreto()
    {
        // La columna del secreto la escribe el vault y nadie mas: este repositorio ni la nombra.
        var (db, repo) = await CreateAsync();
        using var _ = db;
        var c = new Connection(Guid.NewGuid(), "S", Protocol.Ssh, "h") { TieneSecreto = true };
